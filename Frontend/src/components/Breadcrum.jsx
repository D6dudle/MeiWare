  ...
  return (
    <div className=" flex flex-row justify-start items-center  h-[2.5rem] pb-4 rounded-sm text-gray3 ">
      {breadcrumbs.slice(1).map(({ match, breadcrumb }, index, row) => (
        <NavLink key={match.pathname} to={match.pathname == "/home/forum" ? "/home/forum/pesquisar" : match.pathname} onClick={event => handleLinkClick(match.pathname == "/home/forum" ? "/home/forum/pesquisar" : match.pathname)} className={`flex mr-2 ${index + 1 === row.length ? 'text-white': ''}`}>
          {breadcrumb}
